...
profileStruct = profile('info');
[flopTotalPCA,Details]  = FLOPS('Eigenface_f','../results/Eigenface_f.mat',profileStruct);
profile off
...
toc
profileStruct = profile('info');
[flopTotalRBD,Details]  = FLOPS('RBD','../results/RBD.mat',profileStruct);%
profile off
...
toc
profileStruct = profile('info');
[flopTotalL2_CRC,Details]  = FLOPS('L2_CRC','../results/L2_CRC.mat',profileStruct);%
profile off
...
toc
profileStruct = profile('info');
[flopTotalProCRC,Details]  = FLOPS('ProCRC','../results/ProCRC.mat',profileStruct);%
profile off
...
toc
profileStruct = profile('info');
[flopTotalProMax,Details]  = FLOPS('ProMax','../results/ProMax.mat',profileStruct);%
profile off

flopTotalProCRCTotal = flopTotalProCRC+flopTotalProMax;  %% Summing the FLOP counts

accuracy = (sum(pred_tt_label == data.tt_label)) / length(data.tt_label);
cMat1 = confusionmat(otestlabel,pred_tt_label');

fprintf('Accuracy = %f\n', accuracy);
fileID3 = fopen('../results/FinalResults_AS3_bicubic.txt','w');
fprintf(fileID3,'FLOPCOUNTS OF RBD %d\n FLOPCOUNTS OF PCA %d \n',flopTotalRBD, flopTotalPCA)
fprintf(fileID3,'FLOPCOUNTS OF Pro-CRC %d\n FLOPCOUNTS OF L2CRC %d \n',flopTotalProCRCTotal, flopTotalL2_CRC )
